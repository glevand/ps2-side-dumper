 ...
 *
 *  $Id: portable-ohci-core.c,v 1.4 2002/01/10 11:04:26 glevand Exp $
 */
...
#if defined(CONFIG_USB_PS2_OHCI)
# include "portable-ohci-ps2.h"
#elif defined(CONFIG_PMAC_PBOOK)
# include "portable-ohci-pbook.h"
#endif
...
static int sohci_return_urb (urb_t * urb)
{
	...
	Portable_PrepareUrbOnReturn(urb);

#ifdef DEBUG
	urb_print (urb, "RET", usb_pipeout (urb->pipe));
#endif

	SideDumper_OnReturn(urb);
...
//		return -EPIPE;
	...
	ret = Portable_PrepareUrbOnSubmit(urb);
	if(ret) 
	{
		usb_dec_dev_use (urb->dev);	
		urb_rm_priv (urb);
...
{
	struct ohci_device*const dev = Portable_AllocateDevice();
...
{	 
	...
	ed->state = ED_OPER;
	ed->hwNextED = 0;
	sync_and_flush((const void*)(&ed->hwNextED));
...
static void td_fill (unsigned int info, void * data, int len, urb_t * urb, int index)
{
	...
	td_pt->hwNextTD = 0;
	sync_and_flush((const void*)(&td_pt->hwNextTD));
...
{ 
...
#ifdef CONFIG_PS2
	void * ctrl = urb_priv->portable.setup_packet;
	void * data = urb_priv->portable.transfer_buffer;
#else
	...
	switch (usb_pipetype (urb->pipe)) {
		case PIPE_BULK:
			...
			cnt++;
			sync_and_flush((const long*)(data) + data_len / 4 - 1);
...
#ifdef CONFIG_PS2
 				if (td->hwCBP == 0)
					urb->actual_length = bus_to_virt (tdBE) - urb_priv->portable.transfer_buffer + 1;
  				else
					urb->actual_length = bus_to_virt (tdCBP) - urb_priv->portable.transfer_buffer;
#else
 				if (td->hwCBP == 0)
...
static void hc_interrupt (int irq, void * __ohci, struct pt_regs * r)
{
	...
	if ((ohci->hcca.done_head != 0) && !(le32_to_cpup (&ohci->hcca.done_head) & 0x01)) {
 	...
 	ints |= (readl (&regs->intrstatus) & readl (&regs->intrenable));
	if (ints == 0) return;
#else
	if ((ohci->hcca.done_head != 0) && !(le32_to_cpup (&ohci->hcca.done_head) & 0x01)) {
		ints =  OHCI_INTR_WDH;
